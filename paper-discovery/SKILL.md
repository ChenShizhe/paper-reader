...
- **Output:** Files at `$PAPER_BANK/raw/<cite_key>/`; statuses updated in `acquisition-list.md`. Papers with no arXiv ID or that return HTTP 404 first go through the **open-access fallback** (see below); only if that also fails are they marked `manual-pending` in `acquisition-list.md` and appended to `$PAPER_BANK/manual-download-list.md`. On subsequent runs the script auto-detects manually placed files and promotes them to `downloaded`.
- **Open-access fallback (`oa_fetch.py`):** for any paper not retrievable from arXiv, `download_papers.py` calls a resolver cascade — Crossref (title→DOI when needed) → Unpaywall → OpenAlex → PMC eutils (full JATS XML) → Europe PMC (hosted PDF). Cloudflare-gated URLs (e.g. bioRxiv) are fetched with `curl_cffi` Chrome TLS-fingerprint impersonation. Result is saved into `raw/<cite_key>/` as a `.pdf` or `.jats.xml`, with `source_type` `oa-pdf` / `oa-jats-xml`. This means most bioRxiv / PMC / OA-journal papers now acquire automatically; only truly paywalled papers (no preprint, no OA copy) fall to the manual list. Interactive JavaScript challenges (Turnstile) are the only automated miss — those need a browser agent. The import is defensive: if `requests`/`curl_cffi` are absent, the fallback silently disables and arXiv-only behavior is unchanged. Standalone use: `python3 oa_fetch.py --doi <doi> --out <dir> --name <cite_key>` (or `--title`, or `--arxiv`).
